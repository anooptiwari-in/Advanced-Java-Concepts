package io;

import java.io.File;
import java.io.FileNotFoundException;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;

public class TestReaderWriter {

	public static void main(String[] args) {
		// Reading the file...
		String filePath = "D:\\JavaTraining\\CoreJavaProject\\data"; // '\n'
		String fileInputName = "Test.txt";
		String fileOutputName = "TestOutput.txt";
		File fileInput = new File(filePath+File.separator+fileInputName);  // Does not open a file.
		File fileOutput = new File(filePath+File.separator+fileOutputName);
		
		FileReader fis = null;
		FileWriter fos = null;
		
		try {
			// Opens a file
			fis = new FileReader(fileInput);  // Constructor throws java.io.FileNotFoundException- Thrown when wrong path/filename is given.
			// If it fails in opening the file, the 'fis' holds null.
			fos = new FileWriter(fileOutput, true);  // Second parameter for appending a file.
			
			// Traverse a file
			int charContent = fis.read(); // IOException is checked exception.. Statement reads first character from file in int type.
			
			while(charContent != -1){
				System.out.print((char)charContent);
				fos.write(charContent); // Writing into another file.
				charContent = fis.read();
			}
			
		} catch (FileNotFoundException  e) {
			e.printStackTrace();
		}  catch (IOException e) {  // Super catch block can catch sub class exceptions.
			e.printStackTrace();
		} catch (Exception e){  // Generic exception. Should be written at last.
			System.out.println("Unknown exception thrown");
		} finally {
			try {
				if (fis != null)
					fis.close();
				if (fos != null)
					fos.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	}
}
